...
func TestResolveWorktreeBaseDir(t *testing.T) {
	tests := []struct {
		...
		{"relative base_dir", "/repo", &WorktreeConfig{BaseDir: "../wt"}, "/wt"},
		{"absolute base_dir", "/repo", &WorktreeConfig{BaseDir: "/tmp/wt"}, "/tmp/wt"},
		{"empty base_dir uses default", "/repo", &WorktreeConfig{}, "/worktrees"},
	}
...
func TestRunPostCreateHooks_Command(t *testing.T) {
	...
	hooks := []WorktreeHook{{Command: "echo done > marker.txt"}}
...
func TestRunHooks_PreRemoveCommand(t *testing.T) {
	...
	hooks := []WorktreeHook{{Command: "echo cleanup > done.txt"}}
...
func TestRunHooks_FailingCommand(t *testing.T) {
	hooks := []WorktreeHook{{Command: "exit 1"}}
...
func TestResolveBranch_Local(t *testing.T) {
	...
	branch, _ := gitCmd(repo, "rev-parse", "--abbrev-ref", "HEAD")
...
func TestDeleteBranch(t *testing.T) {
	...
	// Verify it's gone.
	_, err = gitCmd(repo, "show-ref", "--verify", "--quiet", "refs/heads/to-delete")
	if err == nil {
...
func TestResolveWorktreeSymbol_Caret(t *testing.T) {
	...
	currentBranch, _ := gitCmd(repo, "rev-parse", "--abbrev-ref", "HEAD")
...
func TestIsBranchIntegrated(t *testing.T) {
	...
	gitCmd(repo, "branch", "same-commit")
...
func TestIsBranchIntegrated_Diverged(t *testing.T) {
	...
	gitCmd(repo, "checkout", "-b", "diverged")
	cmd := exec.Command("git", "-C", repo, "commit", "--allow-empty", "-m", "diverge")
	cmd.Run()
	gitCmd(repo, "checkout", defBranch)
...
func TestResolveWorktreeSymbol_AtDetachedHead(t *testing.T) {
	repo := initTestRepo(t)
	sha, _ := gitCmd(repo, "rev-parse", "HEAD")
...
func TestIsBranchIntegrated_CherryPicked(t *testing.T) {
	...
	featureSHA, _ := gitCmd(repo, "rev-parse", "HEAD")
...
func TestIsBranchIntegrated_PartialCherryPick(t *testing.T) {
	...
	firstSHA, _ := gitCmd(repo, "rev-parse", "HEAD")
...
func TestResolveBranch_Remote(t *testing.T) {
	...
	defBranch, _ := gitCmd(clone, "rev-parse", "--abbrev-ref", "HEAD")
...
func TestHasAgentProcess_NoChildren(t *testing.T) {
	pt := procTable{
		procs:    map[int]procEntry{1: {pid: 1, ppid: 0, comm: "fish", args: "fish"}},
		children: map[int][]int{},
	}
	if hasAgentProcess(pt, 1) {
...
func TestHasAgentProcess_EmptyProcTable(t *testing.T) {
	pt := procTable{procs: map[int]procEntry{}, children: map[int][]int{}}
