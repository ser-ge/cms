...
func worktreeList() error {
	...
	for _, wt := range wts {
		...
		// Use path + separator to avoid prefix-matching parent dirs
		// (e.g. bare repo root is parent of all worktrees).
		marker := " "
...
func worktreeAdd(args []string) error {
	...
	// For remote branches, tracking is already set up via --track.
	// New branches are local-only — user pushes when ready.
