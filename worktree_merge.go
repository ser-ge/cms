...
// worktreeMerge implements the full merge workflow inspired by Worktrunk:
...
func worktreeMerge(args []string) error {
	...
	currentBranch, err := gitCmd(root, "rev-parse", "--abbrev-ref", "HEAD")
	if err != nil {
	...
	if _, err := gitCmd(root, "show-ref", "--verify", "--quiet", "refs/heads/"+target); err != nil {
	...
	// We need to merge from the target branch's worktree (or check it out).
	mergeDir := root
	if targetWt != nil {
		...
		mergeDir = mainWt
		if _, err := gitCmd(mergeDir, "checkout", target); err != nil {
	...
	fmt.Fprintf(os.Stderr, "merging %s into %s\n", currentBranch, target)
	if _, err := gitCmd(mergeDir, mergeArgs...); err != nil {
		if !opts.NoFF {
...
func cleanupTmuxWindow(wtPath string) {
	...
	out, err := runTmux("list-panes", "-a", "-F", "#{session_name}:#{window_index}\t#{pane_current_path}")
	if err != nil {
		return
	}
	...
	for _, line := range strings.Split(out, "\n") {
		...
		if strings.HasPrefix(panePath, wtPath) && !killed[winTarget] {
			runTmux("kill-window", "-t", winTarget)
...
func switchToTmuxWindow(windowName string) {
	...
	runTmux("select-window", "-t", target.Session+":"+windowName)
}
