...
internal/
  ...
  worktree/
    ...
    land.go                       Land, LandOpts, landMergeAndCleanup, squashCommits, resolveBranchesAndWorktrees, commit msg gen
...
- **Active is always computed.** Every item gets `Active` set based on its "live presence" (sessions: attached, worktrees: dirty/ahead or has pane and not merged, projects: has session, panes: has agent, windows: has agent, branches: has worktree, queue: unseen events, marks: pane alive). The `active` sort key controls only sort order, not whether Active is computed.
...
- **Agent summaries use icon+count.** Aggregate sections (sessions, windows, worktrees) show compact `?1 · ⚡2 · ●1` counts, not provider labels. `show_context_percentage` on `[finder]` controls whether max context% appears in summaries.
